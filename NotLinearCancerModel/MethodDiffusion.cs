                    ...
                    for(int j = 0; j < N; j++)
                    {
                        for(int k = 0; k < N; k++)
                        {
                            ...
                            float S3 = (tau / (h * h)) * (d.get(i * h, j * h, k * h, length) * (valuesP1[i + 1, j, k] - valuesP1[i, j, k]) -
                                                    d.get(i * h, j * h, k * h, length) * (valuesP1[i, j, k] - valuesP1[i - 1, j, k]));
                            float S4 = (tau / (h * h)) * (d.get(i * h, j * h, k * h, length) * (valuesP1[i, j + 1, k] - valuesP1[i, j, k]) -
                                                    d.get(i * h, j * h, k * h, length) * (valuesP1[i, j, k] - valuesP1[i, j - 1, k]));
                            float S5 = (tau / (h * h)) * (K * (valuesP1[i, j, k + 1] - valuesP1[i, j, k]) - K * (valuesP1[i, j, k] - valuesP1[i, j, k - 1]));
                            // sources
